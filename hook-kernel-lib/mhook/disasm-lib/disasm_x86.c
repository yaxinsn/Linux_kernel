...
{ \
	...
	{ \
		if (!SuppressErrors){	\
				DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Unexpected operand size prefix\n", VIRTUAL_ADDRESS)); \
		}		\
...
{ \
	...
	{ \
		if (!SuppressErrors){	\
			DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Unexpected address size prefix\n", VIRTUAL_ADDRESS)); \
		}	\
		Instruction->AnomalyOccurred = TRUE; \
	} \
...
#define SANITY_CHECK_SEGMENT_OVERRIDE() \
	...
	{ \
		if (!SuppressErrors) {	\
			DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Unexpected segment override\n", VIRTUAL_ADDRESS)); \
		}	\
		Instruction->AnomalyOccurred = TRUE; \
	}
...
#ifdef WIN32
	#define OUTPUT_ERROR_SEGMENT(x, y) \
		DISASM_OUTPUT(("[0x%08I64X] ANOMALY: unexpected segment 0x%02X\n", (x), (y)));
#else
	#define OUTPUT_ERROR_SEGMENT(x, y) \
		DISASM_OUTPUT(("[0x%08llx] ANOMALY: unexpected segment 0x%02X\n", (x), (y)));
#endif
...
{ \
	...
	{ \
		...
		{ \
			if (!SuppressErrors){ \
				OUTPUT_ERROR_SEGMENT(VIRTUAL_ADDRESS, X86Instruction->Selector); \
			}	\
			Instruction->AnomalyOccurred = TRUE; \
		} \
	} \
	...
	{ \
		switch ((int)X86Instruction->Segment) \
		{ \
			case SEG_CS: \
			case SEG_DS: \
			case SEG_SS: \
			case SEG_ES: \
				...
				Operand->TargetAddress = (U64)(ULONG_PTR)X86Instruction->Displacement; \
			...
			case SEG_GS: \
				assert(!X86Instruction->HasSelector); \
				Operand->TargetAddress = (U64)(ULONG_PTR)GetAbsoluteAddressFromSegment((BYTE)X86Instruction->Segment, (DWORD)X86Instruction->Displacement); \
				break; \
			default: \
...
BOOL X86_GetInstruction(INSTRUCTION *Instruction, U8 *Address, U32 Flags)
{
	...
	while (TRUE)
	{
		...
		if (IS_AMD64() && (Opcode >= REX_PREFIX_START && Opcode <= REX_PREFIX_END) && X86_PREFIX((&X86_Opcodes_1[*Address])))
		{
			if (!Instruction->AnomalyOccurred)
			{
				if (!SuppressErrors) {
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ANOMALY: REX prefix before legacy prefix 0x%02X\n", VIRTUAL_ADDRESS, Opcode));
					#else
						DISASM_OUTPUT(("[0x%08llx] ANOMALY: REX prefix before legacy prefix 0x%02X\n", VIRTUAL_ADDRESS, Opcode));
					#endif
				}
				Instruction->AnomalyOccurred = TRUE;
			}
		...
		if (X86_PREFIX(X86Opcode))
		{
			if (!Instruction->AnomalyOccurred)
			{
				for (i = 0; i < Instruction->PrefixCount; i++)
				{
					if (Instruction->Prefixes[i] == Opcode)
					{
						if (!SuppressErrors) {
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Duplicate prefix 0x%02X\n", VIRTUAL_ADDRESS, Opcode));
						#else
							DISASM_OUTPUT(("[0x%08llx] ANOMALY: Duplicate prefix 0x%02X\n", VIRTUAL_ADDRESS, Opcode));
							KTsgDebug("[0x%08llx] ANOMALY: Duplicate prefix 0x%02X\n", VIRTUAL_ADDRESS, Opcode);
						#endif
						}
						Instruction->AnomalyOccurred = TRUE;
						break;
			...
			switch ((int)Opcode)
			{
				case PREFIX_REPNE: // may be three byte opcode
					...
					if (!Instruction->AnomalyOccurred && X86Instruction->HasRepeatWhileEqualPrefix)
					{
						if (!SuppressErrors) {
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Conflicting prefix\n", VIRTUAL_ADDRESS));
						#else
							DISASM_OUTPUT(("[0x%08llx] ANOMALY: Conflicting prefix\n", VIRTUAL_ADDRESS));
						#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
				...
				case PREFIX_OPERAND_SIZE: // may be three byte opcode
					...
					if (X86Instruction->HasOperandSizePrefix){
						...
						X86Instruction->HasOperandSizePrefix = TRUE;
					}
					switch (X86Instruction->OperandSize)
					{
						case 4: X86Instruction->OperandSize = 2; break;
						case 2: X86Instruction->OperandSize = 4; break;
						default:
							KTsgDebug("[-] X86Instruction->OperandSize: %d\n", X86Instruction->OperandSize);
							assert(0);
							goto abort;
					}
					break;
				...
				case PREFIX_SEGMENT_OVERRIDE_ES:
					...
					else if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors){
							#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Meaningless segment override\n", VIRTUAL_ADDRESS));
							#else
							DISASM_OUTPUT(("[0x%08llx] ANOMALY: Meaningless segment override\n", VIRTUAL_ADDRESS));
							#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
			...
			if (Instruction->PrefixCount >= X86_MAX_INSTRUCTION_LEN)
			{
				if (!SuppressErrors){
					#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ERROR: Reached maximum prefix count %d\n", VIRTUAL_ADDRESS, X86_MAX_PREFIX_LENGTH));
					#else
					DISASM_OUTPUT(("[0x%08llx] ERROR: Reached maximum prefix count %d\n", VIRTUAL_ADDRESS, X86_MAX_PREFIX_LENGTH));
					#endif
				}
				goto abort;
			}
			else if (Instruction->PrefixCount == X86_MAX_PREFIX_LENGTH)
			{
				if (!SuppressErrors) {
					#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Reached maximum prefix count %d\n", VIRTUAL_ADDRESS, X86_MAX_PREFIX_LENGTH));
					#else
					DISASM_OUTPUT(("[0x%08llx] ANOMALY: Reached maximum prefix count %d\n", VIRTUAL_ADDRESS, X86_MAX_PREFIX_LENGTH));
					#endif
				}
				Instruction->AnomalyOccurred = TRUE;
			}
	...
	if (IS_AMD64() && (Opcode >= REX_PREFIX_START && Opcode <= REX_PREFIX_END))
	{
		...
		else if (X86Instruction->rex_b == REX_PREFIX_START)
		{
			if (!Instruction->AnomalyOccurred)
			{
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ANOMALY: meaningless REX prefix used\n", VIRTUAL_ADDRESS));
					#else
						DISASM_OUTPUT(("[0x%08llx] ANOMALY: meaningless REX prefix used\n", VIRTUAL_ADDRESS));
					#endif
				}
				Instruction->AnomalyOccurred = TRUE;
			}
	...
	if (X86_INVALID(X86Opcode))
	{
		if (!SuppressErrors) {
			#ifdef WIN32
				DISASM_OUTPUT(("[0x%08I64X] ERROR: Invalid opcode 0x%02X\n", VIRTUAL_ADDRESS, Opcode));
			#else
				DISASM_OUTPUT(("[0x%08llx] ERROR: Invalid opcode 0x%02X\n", VIRTUAL_ADDRESS, Opcode));
			#endif
		}
		KTsgDebug("[-] goto abort 2\n");
		goto abort;
	}
	...
	if (Opcode == X86_TWO_BYTE_OPCODE)
	{
		...
		KTsgDebug("[*] Opcode: %x => X86Opcode:%x\n", (unsigned int)Opcode, (unsigned int)X86Opcode);
		//
		// Check for errors
		//
		if (X86_INVALID(X86Opcode))
		{
			if (!SuppressErrors){
				#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ERROR: Invalid two byte opcode 0x%02X 0x%02X\n", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode));
				#else
					DISASM_OUTPUT(("[0x%08llx] ERROR: Invalid two byte opcode 0x%02X 0x%02X\n", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode));
				#endif
			}
			KTsgDebug("[-] goto abort 3\n");
			goto abort;
		}
		...
		if (X86Instruction->AddressSize == 8)
		{
			if (X86_Invalid_Addr64_2[Opcode])
			{
				if (!SuppressErrors){
					#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ERROR: Opcode 0x%02X 0x%02X (\"%s\") illegal in 64-bit mode\n", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, X86Opcode->Mnemonic));
					#else
					DISASM_OUTPUT(("[0x%08llx] ERROR: Opcode 0x%02X 0x%02X (\"%s\") illegal in 64-bit mode\n", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, X86Opcode->Mnemonic));
					#endif
				}
				KTsgDebug("[-] goto abort 4\n");
				goto abort;
			}
#if 0
					 ...
					 GET_REX_W(X86Instruction->rex_b) && !GET_REX_W(X86_REX_2[Opcode])))
			{
				if (!SuppressErrors){
					#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ERROR: Illegal REX prefix 0x%02X for opcode 0x%02X 0x%02X\n", VIRTUAL_ADDRESS, X86Instruction->rex_b, X86_TWO_BYTE_OPCODE, Opcode));
					#else
					DISASM_OUTPUT(("[0x%08llx] ERROR: Illegal REX prefix 0x%02X for opcode 0x%02X 0x%02X\n", VIRTUAL_ADDRESS, X86Instruction->rex_b, X86_TWO_BYTE_OPCODE, Opcode));
					#endif
				}
				assert(0);
				goto abort;
		...
		if (X86Instruction->OperandSize == 2 && X86_Invalid_Op16_2[Opcode])
		{
			if (!SuppressErrors) {
				#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ERROR: Opcode 0x%02X 0x%02X (\"%s\") illegal with 16-bit operand size\n", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, X86Opcode->Mnemonic));
				#else
					DISASM_OUTPUT(("[0x%08llx] ERROR: Opcode 0x%02X 0x%02X (\"%s\") illegal with 16-bit operand size\n", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, X86Opcode->Mnemonic));
				#endif
			}
			KTsgDebug("[-] goto abort 5\n");
			goto abort;
		}
		...
		if (X86_SPECIAL_EXTENSION(X86Opcode))
		{
			...
			KTsgDebug("[-] goto HasSpecialExtension 1\n");
		...
		else if (SSE_Prefix && !X86_INVALID(&X86_SSE[Opcode])) // SSEx instruction
		{
			...
			if (IS_X86_16())
			{
				if (!SuppressErrors) {
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ERROR: SSE invalid in 16-bit mode\n", VIRTUAL_ADDRESS));
					#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: SSE invalid in 16-bit mode\n", VIRTUAL_ADDRESS));
					#endif
				}
				KTsgDebug("[-] goto abort 6\n");
				goto abort;
			}

			assert(X86Instruction->HasModRM);
			switch ((int)SSE_Prefix)
			{
			...
			if (X86_INVALID(X86Opcode))
			{
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ERROR: Illegal SSE instruction opcode 0x%02X 0x%02X + prefix 0x%02X\n", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Instruction->OpcodeBytes[2]));
					#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: Illegal SSE instruction opcode 0x%02X 0x%02X + prefix 0x%02X\n", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Instruction->OpcodeBytes[2]));
					#endif
				}
				KTsgDebug("[-] goto abort 7\n");
				goto abort;
			}
			else if (X86_EXTENDED_OPCODE(X86Opcode))
			{
				...
				if (X86_INVALID(X86Opcode))
				{
					if (!SuppressErrors){
						#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ERROR: Illegal SSE instruction opcode 0x%02X 0x%02X + prefix 0x%02X + extension %d\n", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Instruction->OpcodeBytes[2], OpcodeExtension));
						#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: Illegal SSE instruction opcode 0x%02X 0x%02X + prefix 0x%02X + extension %d\n", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Instruction->OpcodeBytes[2], OpcodeExtension));
						#endif
					}
					KTsgDebug("[-] goto abort 8\n");
					goto abort;
				}
			}
			...
			X86Instruction->HasRepeatWhileNotEqualPrefix = FALSE;
			X86Instruction->HasOperandSizePrefix = FALSE;
			switch ((int)X86_GET_CATEGORY(X86Opcode))
			{
				case ITYPE_SSE: case ITYPE_SSE2: case ITYPE_SSE3:
					break;
				default:
					KTsgDebug("[-] goto abort 9\n");
					assert(0);
					goto abort;
			}
		}
		else if (X86_EXTENDED_OPCODE(X86Opcode)) // 2 byte group
		{
			...
			if (X86_INVALID(X86Opcode))
			{
				...
				if (!SuppressErrors) {
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ERROR: Invalid group opcode 0x%02X 0x%02X extension 0x%02X\n", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, OpcodeExtension));
					#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: Invalid group opcode 0x%02X 0x%02X extension 0x%02X\n", VIRTUAL_ADDRESS, X86_TWO_BYTE_OPCODE, Opcode, OpcodeExtension));
					#endif
				}
				KTsgDebug("[-] goto abort 10\n");
				goto abort;
			}
	...
	else // 1-byte opcode
	{
		if (X86Instruction->AddressSize == 8)
		{
			if (X86_Invalid_Addr64_1[Opcode])
			{
				if (!SuppressErrors){
					#ifdef WIN32
						printf("[0x%08I64X] ERROR: Opcode 0x%02X (\"%s\") illegal in 64-bit mode\n", VIRTUAL_ADDRESS, Opcode, X86Opcode->Mnemonic);
					#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: Opcode 0x%02X (\"%s\") illegal in 64-bit mode\n", VIRTUAL_ADDRESS, Opcode, X86Opcode->Mnemonic));
					#endif
				}
				KTsgDebug("[-] goto abort 11\n");
				goto abort;
			}
		...
		if (X86Instruction->OperandSize == 2 && X86_Invalid_Op16_1[Opcode])
		{
			if (!SuppressErrors){
			#ifdef WIN32
				printf("[0x%08I64X] ERROR: Opcode 0x%02X (\"%s\") illegal with 16-bit operand size\n", VIRTUAL_ADDRESS, Opcode, X86Opcode->Mnemonic);
			#else
				DISASM_OUTPUT(("[0x%08llx] ERROR: Opcode 0x%02X (\"%s\") illegal with 16-bit operand size\n", VIRTUAL_ADDRESS, Opcode, X86Opcode->Mnemonic));
			#endif
			}
			KTsgDebug("[-] goto abort 12\n");
			goto abort;
		}
		...
		if (X86_EXTENDED_OPCODE(X86Opcode)) // a group
		{
			...
			#endif
				SpecialExtension = TRUE;
				KTsgDebug("[-] goto HasSpecialExtension 3\n");
				goto HasSpecialExtension;
			}
			...
			if (X86_INVALID(X86Opcode))
			{
				...
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ERROR: Invalid group opcode 0x%02X extension 0x%02X\n", VIRTUAL_ADDRESS, Opcode, OpcodeExtension));
					#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: Invalid group opcode 0x%02X extension 0x%02X\n", VIRTUAL_ADDRESS, Opcode, OpcodeExtension));
					#endif
				}
				KTsgDebug("[-] goto abort 13\n");
				goto abort;
			}
		...
		else
		{
			if (X86_SPECIAL_EXTENSION(X86Opcode))
			{
				...
				SpecialExtension = TRUE;
				KTsgDebug("[-] goto HasSpecialExtension 4\n");
				goto HasSpecialExtension;
			}
			#ifdef WIN32
...
HasSpecialExtension:
	if (SpecialExtension)
	{
		if (X86Opcode->MnemonicFlags & ITYPE_EXT_MODRM)
		{
			...
			if (X86_INVALID(X86Opcode))
			{
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ERROR: Illegal opcode 0x%02X 0x%02X + modrm 0x%02X\n", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], *Address));
					#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: Illegal opcode 0x%02X 0x%02X + modrm 0x%02X\n", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], *Address));
					#endif
				}
				goto abort;
			}
		...
		else if (X86Opcode->MnemonicFlags & ITYPE_EXT_FPU)
		{
			...
			if (X86_INVALID(X86Opcode))
			{
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ERROR: Invalid FPU opcode 0x%02X + modrm extension 0x%02X (index 0x%02X)\n", VIRTUAL_ADDRESS, Opcode, X86Instruction->modrm_b, 0x08 + OpcodeExtension));
					#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: Invalid FPU opcode 0x%02X + modrm extension 0x%02X (index 0x%02X)\n", VIRTUAL_ADDRESS, Opcode, X86Instruction->modrm_b, 0x08 + OpcodeExtension));
					#endif
				}
				goto abort;
			}
		...
		else if (X86Opcode->MnemonicFlags & ITYPE_EXT_SUFFIX)
		{
			if (X86Instruction->HasOperandSizePrefix)
			{
				if (!Instruction->AnomalyOccurred && X86Opcode->Table == X86_3DNOW_0F)
				{
					if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ANOMALY: operand size prefix used with 3DNOW instruction\n", VIRTUAL_ADDRESS));
						#else
							DISASM_OUTPUT(("[0x%08llx] ANOMALY: operand size prefix used with 3DNOW instruction\n", VIRTUAL_ADDRESS));
						#endif
					}
					Instruction->AnomalyOccurred = TRUE;
				}
			...
			if (X86_INVALID(X86Opcode))
			{
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ERROR: Illegal opcode 0x%02X 0x%02X + suffix 0x%02X\n", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Suffix));
					#else
						DISASM_OUTPUT(("[0x%08llx] ERROR: Illegal opcode 0x%02X 0x%02X + suffix 0x%02X\n", VIRTUAL_ADDRESS, Instruction->OpcodeBytes[0], Instruction->OpcodeBytes[1], Suffix));
					#endif
				}
				goto abort;
			}
	...
	if (IS_X86_16() && X86Opcode->CPU > CPU_I386)
	{
		if (!SuppressErrors){
			#ifdef WIN32
				DISASM_OUTPUT(("[0x%08I64X] ERROR: Instruction \"%s\" (opcode 0x%02X) can't be used in 16-bit X86\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->LastOpcode));
			#else
				DISASM_OUTPUT(("[0x%08llx] ERROR: Instruction \"%s\" (opcode 0x%02X) can't be used in 16-bit X86\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->LastOpcode));
			#endif
		}
		goto abort;
	}
	if (!IS_AMD64() && X86Opcode->CPU >= CPU_AMD64)
	{
		if (!SuppressErrors){
			#ifdef WIN32
				DISASM_OUTPUT(("[0x%08I64X] ERROR: Instruction \"%s\" (opcode 0x%02X) can only be used in X86-64\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->LastOpcode));
			#else
				DISASM_OUTPUT(("[0x%08llx] ERROR: Instruction \"%s\" (opcode 0x%02X) can only be used in X86-64\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->LastOpcode));
			#endif
		}
		goto abort;
	}
	...
	if (Instruction->Groups & (ITYPE_FPU|ITYPE_MMX|ITYPE_SSE|ITYPE_SSE2|ITYPE_SSE3))
	{
		...
		for (i = 0; i < Instruction->PrefixCount; i++)
		{
			switch ((int)Instruction->Prefixes[i])
			{
				case PREFIX_OPERAND_SIZE:
					...
					if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors){
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ANOMALY: operand size prefix used with FPU/MMX/SSEx\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ANOMALY: operand size prefix used with FPU/MMX/SSEx\n", VIRTUAL_ADDRESS));
							#endif
						}
						goto abort;
					}
				...
				case PREFIX_REP:
					...
					if (!SuppressErrors){
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ERROR: rep/repne used with MMX/SSEx\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ERROR: rep/repne used with MMX/SSEx\n", VIRTUAL_ADDRESS));
							#endif
					}
					goto abort;
			...
			for (i = 0; i < Instruction->PrefixCount; i++)
			{
				if (Instruction->Prefixes[i] == PREFIX_OPERAND_SIZE)
				{
					...
					if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors){
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ANOMALY: use of operand size prefix meaningless when REX.w=1\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ANOMALY: use of operand size prefix meaningless when REX.w=1\n", VIRTUAL_ADDRESS));
							#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
		...
		if (X86Instruction->HasDefault64Operand)
		{
			if (X86Instruction->rex.w)
			{
				if (!Instruction->AnomalyOccurred)
				{
					if (!SuppressErrors) {
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ANOMALY: use of REX.w is meaningless (default operand size is 64)\n", VIRTUAL_ADDRESS));
					#else
						DISASM_OUTPUT(("[0x%08llx] ANOMALY: use of REX.w is meaningless (default operand size is 64)\n", VIRTUAL_ADDRESS));
					#endif
					}
					Instruction->AnomalyOccurred = TRUE;
				}
		...
		switch ((int)Instruction->Type)
		{
			...
			case ITYPE_STRLOAD:
				if (X86Instruction->HasRepeatWhileNotEqualPrefix)
				{
					if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ANOMALY: REPNE should only be used with cmps/scas\n", VIRTUAL_ADDRESS));
						#else
							DISASM_OUTPUT(("[0x%08llx] ANOMALY: REPNE should only be used with cmps/scas\n", VIRTUAL_ADDRESS));
						#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
			...
			default:
				if (!Instruction->AnomalyOccurred)
				{
					if (!SuppressErrors) {
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Repeat prefix used with non-string instruction\n", VIRTUAL_ADDRESS));
						#else
							DISASM_OUTPUT(("[0x%08llx] ANOMALY: Repeat prefix used with non-string instruction\n", VIRTUAL_ADDRESS));
						#endif
						}
					Instruction->AnomalyOccurred = TRUE;
				}
	...
	if (IS_X86_32())
	{
		...
		{
			#ifdef WIN32
				DISASM_OUTPUT(("[0x%08I64X] WARNING: instruction lengths differ (%d vs %d)\n", VIRTUAL_ADDRESS, Instruction->Length, InstructionLength));
			#else
				DISASM_OUTPUT(("[0x%08llx] WARNING: instruction lengths differ (%d vs %d)\n", VIRTUAL_ADDRESS, Instruction->Length, InstructionLength));
			#endif
	...
	if (!X86Instruction->HasDstAddressing && !X86Instruction->HasSrcAddressing)
	{
		if (X86Instruction->HasAddressSizePrefix)
		{
			if (!Instruction->AnomalyOccurred)
			{
				if (!SuppressErrors) {
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ANOMALY: address size prefix used with no addressing\n", VIRTUAL_ADDRESS));
					#else
						DISASM_OUTPUT(("[0x%08llx] ANOMALY: address size prefix used with no addressing\n", VIRTUAL_ADDRESS));
					#endif
				}
				Instruction->AnomalyOccurred = TRUE;
			}
		...
		if (X86Instruction->HasSegmentOverridePrefix)
		{
			if (!Instruction->AnomalyOccurred)
			{
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ANOMALY: segment override used with no addressing\n", VIRTUAL_ADDRESS));
					#else
						DISASM_OUTPUT(("[0x%08llx] ANOMALY: segment override used with no addressing\n", VIRTUAL_ADDRESS));
					#endif
				}
				Instruction->AnomalyOccurred = TRUE;
			}
	...
	if (!Instruction->AnomalyOccurred && !IS_X86_16())
	{
		switch ((int)X86Instruction->Segment)
		{
			...
			case SEG_ES:
				switch ((int)Instruction->Type)
				{
					...
					default:
						if (!SuppressErrors){
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ANOMALY: use of unexpected segment ES\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ANOMALY: use of unexpected segment ES\n", VIRTUAL_ADDRESS));
							#endif
						}
						Instruction->AnomalyOccurred = TRUE;
						break;
			...
			case SEG_FS:
				...
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ANOMALY: use of unexpected segment FS\n", VIRTUAL_ADDRESS));
					#else
						DISASM_OUTPUT(("[0x%08llx] ANOMALY: use of unexpected segment FS\n", VIRTUAL_ADDRESS));
					#endif
				}
				Instruction->AnomalyOccurred = TRUE;
				break;
			case SEG_GS:
				...
				if (!SuppressErrors){
					#ifdef WIN32
						DISASM_OUTPUT(("[0x%08I64X] ANOMALY: use of unexpected segment GS\n", VIRTUAL_ADDRESS));
					#else
						DISASM_OUTPUT(("[0x%08llx] ANOMALY: use of unexpected segment GS\n", VIRTUAL_ADDRESS));
					#endif
				}
				Instruction->AnomalyOccurred = TRUE;
				break;
			...
			switch ((int)Instruction->Prefixes[i])
			{
				case PREFIX_BRANCH_NOT_TAKEN:
					if (!Instruction->AnomalyOccurred && X86Instruction->Segment != SEG_CS)
					{
						if (!SuppressErrors) {
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Segment override used with conditional branch\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ANOMALY: Segment override used with conditional branch\n", VIRTUAL_ADDRESS));
							#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
		...
		!IsValidLockPrefix(X86Instruction, Opcode, Instruction->OpcodeLength, Group, OpcodeExtension))
	{
		if (!SuppressErrors) {
			#ifdef WIN32
				DISASM_OUTPUT(("[0x%08I64X] ERROR: Illegal use of lock prefix for instruction \"%s\"\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic));
			#else
				DISASM_OUTPUT(("[0x%08llx] ERROR: Illegal use of lock prefix for instruction \"%s\"\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic));
			#endif
		}
		goto abort;
	}
	...
	if (!Instruction->Length || Instruction->Length > X86_MAX_INSTRUCTION_LEN)
	{
		if (!SuppressErrors) {
			#ifdef WIN32
				DISASM_OUTPUT(("[0x%08I64X] ERROR: maximum instruction length reached (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
			#else
				DISASM_OUTPUT(("[0x%08llx] ERROR: maximum instruction length reached (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
			#endif
		}
		goto abort;
	}
	...
	if (Instruction->Groups & ITYPE_EXEC)
	{
		...
		if (!Instruction->AnomalyOccurred &&
			Operand1->TargetAddress >= (U64)(ULONG_PTR)Instruction->Address &&
			(Operand1->TargetAddress < (U64)(ULONG_PTR)Instruction->Address + (U64)(ULONG_PTR)Instruction->Length))
		{
			if (!SuppressErrors) {
				#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ANOMALY: branch into the middle of an instruction\n", VIRTUAL_ADDRESS));
				#else
					DISASM_OUTPUT(("[0x%08llx] ANOMALY: branch into the middle of an instruction\n", VIRTUAL_ADDRESS));
				#endif
			}
			Instruction->AnomalyOccurred = TRUE;
		}
		...
		for (i = 0, Operand = Instruction->Operands; i < Instruction->OperandCount; i++, Operand++)
		{
			if (Operand->TargetAddress)
			{
				if (Operand->Flags & OP_DST)
				{
					...
					#else
						DISASM_OUTPUT(("[0x%08llx] Write of size %d to 0x%04I64X\n", VIRTUAL_ADDRESS, Operand->Length, Operand->TargetAddress));
					#endif
				}
				if (Operand->Flags & OP_SRC)
				{
					...
					#else
						DISASM_OUTPUT(("[0x%08llx] Read of size %d to 0x%04I64X\n", VIRTUAL_ADDRESS, Operand->Length, Operand->TargetAddress));
					#endif
				}
			}
	...
	if (Instruction->Groups & ITYPE_STACK)
	{
		switch ((int)Instruction->Type)
		{
			...
			case ITYPE_ENTER:
				if (!Instruction->AnomalyOccurred)
				{
					if (Instruction->Operands[1].Value_U64 & 3)
					{
						if (!SuppressErrors){
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ANOMALY: ENTER has invalid operand 2\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ANOMALY: ENTER has invalid operand 2\n", VIRTUAL_ADDRESS));
							#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
					if (Instruction->Operands[2].Value_U64 & ~0x1F)
					{
						if (!SuppressErrors){
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ANOMALY: ENTER has invalid operand 3\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ANOMALY: ENTER has invalid operand 3\n", VIRTUAL_ADDRESS));
							#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
				...
				switch (Opcode)
				{
					...
					case 0xC2: // ret with 1 arg
						if (!Instruction->AnomalyOccurred && (Operand1->Value_U64 & 3))
						{
							if (!SuppressErrors) {
								#ifdef WIN32
									DISASM_OUTPUT(("[0x%08I64X] ANOMALY: ret has invalid operand 1\n", VIRTUAL_ADDRESS));
								#else
									DISASM_OUTPUT(("[0x%08llx] ANOMALY: ret has invalid operand 1\n", VIRTUAL_ADDRESS));
								#endif
							}
							Instruction->AnomalyOccurred = TRUE;
						}
						Instruction->StackChange += (S32)Operand1->Value_U64;
						break;
					...
					case 0xCA: // far ret with 1 arg
						if (!Instruction->AnomalyOccurred && (Operand1->Value_U64 & 3))
						{
							if (!SuppressErrors) {
								#ifdef WIN32
									DISASM_OUTPUT(("[0x%08I64X] ANOMALY: retf has invalid operand 1\n", VIRTUAL_ADDRESS));
								#else
									DISASM_OUTPUT(("[0x%08llx] ANOMALY: retf has invalid operand 1\n", VIRTUAL_ADDRESS));
								#endif
							}
							Instruction->AnomalyOccurred = TRUE;
						}
			...
			case ITYPE_ADD:
			case ITYPE_XCHGADD:
				if (Instruction->Operands[1].Value_S64) Instruction->StackChange = (S32)(Instruction->Operands[1].Value_S64);
				break;
			case ITYPE_SUB:
				if (Instruction->Operands[1].Value_S64) Instruction->StackChange = (S32)(-Instruction->Operands[1].Value_S64);
				break;
			...
			default:
				if (!Instruction->AnomalyOccurred)
				{
					if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Instruction \"%s\" is modifying the stack\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ANOMALY: Instruction \"%s\" is modifying the stack\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic));
						#endif
					}
					Instruction->AnomalyOccurred = TRUE;
				}
			...
			((X86Instruction->OperandSize != 2 && (Instruction->StackChange & 3)) || (Instruction->StackChange & 1)))
		{
			if (!SuppressErrors) {
				#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ANOMALY: \"%s\" has invalid stack change 0x%02X\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->StackChange));
				#else
					DISASM_OUTPUT(("[0x%08llx] ANOMALY: \"%s\" has invalid stack change 0x%02X\n", VIRTUAL_ADDRESS, X86Opcode->Mnemonic, Instruction->StackChange));
				#endif
			}
			Instruction->AnomalyOccurred = TRUE;
		}
...
#ifdef TEST_DISASM
		#ifdef WIN32
			DISASM_OUTPUT(("Dump of 0x%04I64X:\n", VIRTUAL_ADDRESS));
		#else
			DISASM_OUTPUT(("Dump of 0x%04llx:\n", VIRTUAL_ADDRESS));
		#endif
	...
	for (OperandIndex = 0; OperandIndex < Instruction->OperandCount; OperandIndex++)
	{
		...
		switch ((int)OperandType)
		{
			...
			case OPTYPE_cpu:
				if (!SuppressErrors){
				#ifdef WIN32
					DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Undocumented loadall instruction?\n", VIRTUAL_ADDRESS));
				#else
					DISASM_OUTPUT(("[0x%08llx] ANOMALY: Undocumented loadall instruction?\n", VIRTUAL_ADDRESS));
				#endif
				}
				Instruction->AnomalyOccurred = TRUE;
				Operand->Length = 204;
			...
			case OPTYPE_p: // 32-bit or 48-bit pointer depending on operand size
				if (!Instruction->AnomalyOccurred && X86Instruction->HasSegmentOverridePrefix)
				{
					if (!SuppressErrors) {
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Segment override used when segment is explicit\n", VIRTUAL_ADDRESS));
						#else
							DISASM_OUTPUT(("[0x%08llxX] ANOMALY: Segment override used when segment is explicit\n", VIRTUAL_ADDRESS));
						#endif
					}
					Instruction->AnomalyOccurred = TRUE;
				}
		...
		switch ((int)AddressMode)
		{
			...
			case AMODE_J: // IP-relative jump offset
				...
				if (Decode)
				{
					...
					if ((Operand->Flags & OP_COND) && !X86Instruction->Displacement)
					{
						if (!SuppressErrors) {
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ANOMALY: Both conditions of branch go to same address\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ANOMALY: Both conditions of branch go to same address\n", VIRTUAL_ADDRESS));
							#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
			...
			case AMODE_Y: // ES:[EDI]
				...
				if (X86Instruction->HasSegmentOverridePrefix)
				{
					if (!Instruction->AnomalyOccurred)
					{
						if (!SuppressErrors) {
							#ifdef WIN32
								DISASM_OUTPUT(("[0x%08I64X] ANOMALY: segment override used with AMODE_Y\n", VIRTUAL_ADDRESS));
							#else
								DISASM_OUTPUT(("[0x%08llx] ANOMALY: segment override used with AMODE_Y\n", VIRTUAL_ADDRESS));
							#endif
						}
						Instruction->AnomalyOccurred = TRUE;
					}
			...
			case AMODE_PR: // modrm.rm = mmx register and modrm.mod = 11
				...
				if (modrm.mod != 3)
				{
					if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: mod != 3 for AMODE_PR (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: mod != 3 for AMODE_PR (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
				else if (rex_modrm.rm > 7)
				{
					if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: invalid mmx register %d for AMODE_PR (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: invalid mmx register %d for AMODE_PR (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
				else if (X86Instruction->OperandSize == 2)
				{
					if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: AMODE_PR illegal in 16-bit mode (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: AMODE_PR illegal in 16-bit mode (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
			...
			case AMODE_VR: // modrm.rm = xmm register and modrm.mod = 11
				...
				if (modrm.mod != 3)
				{
					if (!SuppressErrors) {
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: mod != 3 for AMODE_VR (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: mod != 3 for AMODE_VR (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
				else if (X86Instruction->OperandSize == 2)
				{
					if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: AMODE_VR illegal in 16-bit mode (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: AMODE_VR illegal in 16-bit mode (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.rm, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
			...
			case AMODE_P: // modrm.reg = mmx register
				...
				if (rex_modrm.reg > 7)
				{
					if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: invalid mmx register %d for AMODE_P (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.reg, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: invalid mmx register %d for AMODE_P (\"%s\")\n", VIRTUAL_ADDRESS, rex_modrm.reg, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
				else if (X86Instruction->OperandSize == 2)
				{
					if (!SuppressErrors) {
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: AMODE_P illegal in 16-bit mode (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: AMODE_P illegal in 16-bit mode (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
			...
			case AMODE_R: // modrm.rm is general register and modrm.mod = 11
				...
				if (modrm.mod != 3)
				{
					if (!SuppressErrors) {
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: mod != 3 for AMODE_R (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: mod != 3 for AMODE_R (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
			...
			case AMODE_M: // memory only
				...
				if (modrm.mod == 3)
				{
					if (!SuppressErrors){
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: mod = 3 for AMODE_M (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: mod = 3 for AMODE_M (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
			...
			case AMODE_E: // general register or memory
				...
				if (OperandType == OPTYPE_p && modrm.mod == 3)
				{
					if (!SuppressErrors) {
						#ifdef WIN32
							DISASM_OUTPUT(("[0x%08I64X] ERROR: mod = 3 for AMODE_E with OPTYPE_p (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#else
							DISASM_OUTPUT(("[0x%08llx] ERROR: mod = 3 for AMODE_E with OPTYPE_p (\"%s\")\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic));
						#endif
					}
					goto abort;
				}
	...
	else
	{
		...
		switch (modrm.rm)
		{
			...
			case 2:
				...
				X86Instruction->IndexRegister = X86_REG_SI;
				X86Instruction->HasIndexRegister = TRUE;
				X86_SET_SEG((X86_32BIT_REGISTER)REG_BP);
				break;
			case 3:
	...
	else if (modrm.mod == 0 && modrm.rm == 5)
	{
		...
		if (IS_AMD64())
		{
			...
			for (ImmediateSize = 0, i = OperandIndex+1; i < Instruction->OperandCount; i++)
			{
				...
				switch ((int)(X86Instruction->Opcode.OperandFlags[i] & X86_OPTYPE_MASK))
				{
					case OPTYPE_v:
...
INTERNAL BOOL IsValidLockPrefix(X86_INSTRUCTION *X86Instruction, U8 Opcode, U32 OpcodeLength, U8 Group, U8 OpcodeExtension)
{
	...
	if (!X86Instruction->HasModRM || X86Instruction->modrm.mod == 3 || !X86Instruction->HasDstAddressing)
	{
		...
		#else
		DISASM_OUTPUT(("[0x%08l] ERROR: Instruction \"%s\" with LOCK prefix has invalid ModRM addressing\n", VIRTUAL_ADDRESS, X86Instruction->Opcode.Mnemonic, X86Instruction->Instruction->Address));
		#endif
		return FALSE;
	}
