...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 19) && LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 16) // no check-linux-src haozelong for redhat5.4
...
int find_symbol_address_from_file(struct kernsym *sym, const char *filename) {
	...
	if (IS_ERR(f)) {
		ret = PTR_ERR(f);
		KTsgDebug(PKPRE "Unable to open file %s\n", filename);
		goto out_nofilp;
	}
	...
	while (vfs_read(f, p+i, 1, &f->f_pos) == 1) {
		...
		if (p[i] == '\n' || i == (MAX_FILE_LEN-1)) {
			...
			if (sym->found) {
				...
				//KTsgDebug(PKPRE "From %s, found %s end addr at %lx (total size %lu)\n", filename, sym->name, sym->end_addr, sym->size);
			...
			if (!sym->found && substr != NULL && substr[-1] == ' ' && substr[strlen(sym->name)+1] == '\0') {
				...
				//KTsgDebug(PKPRE "From %s, found %s start addr at %lx\n", filename, sym->name, sym->addr);
...
char* __kernel_release_str(void)
{
...
int find_symbol_address(struct kernsym *sym, const char *symbol_name) {
    ...
    char* pstrRelease = __kernel_release_str();
	...
	if (IN_ERR(ret)) {
		...
		filename = kmalloc(strlen(pstrRelease)+strlen(SYSTEM_MAP_PATH)+1, GFP_KERNEL);
		...
		memset(filename, 0, strlen(SYSTEM_MAP_PATH)+strlen(pstrRelease)+1);

		strncpy(filename, SYSTEM_MAP_PATH, strlen(SYSTEM_MAP_PATH));
		strncat(filename, pstrRelease, strlen(pstrRelease));
...
//	if (IN_ERR(ret))
//		KTsgDebug(PKPRE "Failed to find symbol address for %s\n", symbol_name);
